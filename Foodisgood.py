...
def main():
    ...
    if 'username' not in st.session_state:
        ...
        st.subheader("Bitte gib deinen Benutzernamen ein, um fortzufahren:")
        username = st.text_input("Benutzername eingeben:")
        if username:
            st.session_state.username = username
            st.success("Benutzername gespeichert! Klicke unten, um fortzufahren.")

    # If username is entered, show the main app
    if 'username' in st.session_state and st.session_state.username:
        st.title(f"Willkommen zurück, {st.session_state.username} bei FoodisGood")
        
        # Tabs für Rezeptverwaltung und Nahrungsergänzungsmittel
        tab_options = ["Rezepte anzeigen", "Rezepte hinzufügen", "Rezepte löschen"]
        ...
        if selected_tab == "Rezepte anzeigen":
            ...
            st.markdown(f"<div style='font-size: 18px'><b>Anleitung:</b> {selected_recipe_info['Anleitung']}</div>", unsafe_allow_html=True)
            ...
            st.image(selected_recipe_info["Bild"], caption="Bild des Gerichts", use_column_width=True)
